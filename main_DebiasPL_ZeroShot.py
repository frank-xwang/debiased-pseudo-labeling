...
def CLDLoss(prob_s, prob_w, mask=None, weights=None):
    # criterion_cld = torch.nn.CrossEntropyLoss().cuda()
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    try:
        ...
        trainpseudo_labels_x = index_info_x['PseudoLabel'].tolist()

        index_info_u = pd.read_csv(index_info_u)
        trainindex_u = index_info_u['Index'].tolist()
        trainpseudo_labels_u = index_info_u['PseudoLabel'].tolist()
        trainprobs_u = index_info_u['Probs'].tolist()
        ...
        train_dataset_x, train_dataset_u, val_dataset = get_imagenet_ssl(
            ...
            multiviews=args.multiviews, PseudoLabels_x=trainpseudo_labels_x)
    except:
        print("cannot find trainindex_x and trainindex_u")
    ...
    clip_preds_list = torch.tensor(trainpseudo_labels_u, dtype=torch.long).cuda()
...
def train(train_loader_x, train_loader_u, model, optimizer, epoch, args, qhat=None, writer=None, \
    ...
    for i, (images_u, targets_u, indexs_u) in enumerate(train_loader_u):
        ...
        pseudo_label_u_clip = clip_preds_list[indexs_u].view(-1)
        pseudo_label_u_clip = pseudo_label_u_clip.cuda(args.gpu, non_blocking=True)
        # add mask for clip with thresholding
        probs_list = clip_probs_list[indexs_u].cuda(args.gpu, non_blocking=True)
        mask_clip = probs_list.ge(0.3).float()
        # apply clip predictions to low-confidence predictions
        if args.multiviews:
            mask_delta = (mask_clip - mask - mask1 - mask2).ge(0.01).float()
            loss_u_clip = [F.cross_entropy(logits_u, pseudo_label_u_clip, reduction='none', weight=per_cls_weights) * mask_delta for logits_u in logits_u_list]
            loss_u = (torch.stack(loss_u_clip, dim=0).mean() + loss_u) / 2.0
        else:
            mask_delta = (mask_clip - mask).ge(0.01).float()
            loss_u_clip = F.cross_entropy(logits_u_s, pseudo_label_u_clip, reduction='none', weight=per_cls_weights) * mask_delta
            loss_u = (loss_u_clip + loss_u) / 2.0
